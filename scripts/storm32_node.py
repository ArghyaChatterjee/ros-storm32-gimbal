...
def pub_timer_callback(event):
    ...
    try:
        euler = gimbal.get_imu1_angles()
        if euler:
            rospy.logdebug_throttle(1, ("get_imu1_angles: pitch={0:.2f}, " +
                                        "roll={1:.2f}, yaw={2:.2f}").format(
                                            *euler))
            euler = list(map(np.radians, euler))
            q = tf.transformations.quaternion_from_euler(*euler, axes='syxz')
...
if __name__ == '__main__':
    ...
    frame_id = rospy.get_param("~frame_id", 'gimbal')
    ...
    if version:
        ...
        sub = rospy.Subscriber(
            ...
            queue_size=1)
        pub = rospy.Publisher('~orientation', QuaternionStamped, queue_size=1)
